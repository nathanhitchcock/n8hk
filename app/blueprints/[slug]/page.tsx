...
export async function generateMetadata({ params }: { params: Promise<Params> }) {
  ...
  return {
    ...
    openGraph: {
      ...
      type: 'article',
      publishedTime,
      url: `${baseUrl}/blueprints/${entry.slug}`,
    },
    twitter: {
...
export default async function BlueprintEntryPage({ params }: { params: Promise<Params> }) {
  const { slug } = await params
  ...
  const blueprintName = entry.metadata.blueprint || 'Blueprint'
  const blueprintEntries = allEntries
    .filter((item) => (item.metadata.blueprint || 'Blueprint') === blueprintName)
    .sort((a, b) => a.metadata.step - b.metadata.step)

  const currentIndex = blueprintEntries.findIndex((item) => item.slug === entry.slug)
  const nextEntry = blueprintEntries[currentIndex + 1]
  const tocItems = getTocItems(entry.content)

  const linkedNotes = getFieldNotes().filter(
    (note) => note.metadata.blueprintStep === entry.slug
  )
      ...
      <div className="lg:grid lg:grid-cols-12 lg:gap-12">
        ...
        <article className="order-2 surface-card prose enter-wash lg:prose-lg lg:order-1 lg:col-span-8 rounded-3xl border px-6 py-7 shadow-sm md:px-10 md:py-10">
          ...
          {nextEntry ? (
            <p className="text-sm">
              Next step:{' '}
              <a href={`/blueprints/${nextEntry.slug}`}>
